        ...
        return (
          <li key={s.path} className="relative group">
            ...
            >
              ...
              {(isMobile || !collapsed) && <span className="font-medium whitespace-nowrap">{label}</span>}
            </NavLink>
            {!isMobile && collapsed && (
    ...
    <>
      ...
      >
        <div className="pt-16 px-3 pb-6">
          <p className="text-[10px] text-base-content/30 font-mono px-3 mb-3">{t('nav.navigation')}</p>
          {navLinks(true)}
        </div>
        ...
        <div className="px-6 pb-6 border-t border-base-content/5 pt-4">
          <p className="text-[10px] text-primary/60 font-mono font-medium">{t('brand.subtitle')}</p>
          <p className="text-[9px] text-base-content/30 font-mono mt-0.5">{t('brand.device')}</p>
        </div>
      </aside>
      ...
      >
        <div className={`pt-5 pb-4 transition-[padding] duration-300 ${collapsed ? 'px-2' : 'px-4'}`}>
          <NavLink to="/" className="block">
            ...
            {!collapsed && (
              <>
                <p className="text-[11px] text-primary/60 mt-1.5 font-mono font-medium tracking-wide leading-snug">{t('brand.subtitle')}</p>
                <p className="text-[10px] text-base-content/30 font-mono">{t('brand.device')}</p>
              </>
            )}
          </NavLink>
        </div>
        ...
        <button
          ...
          className={`hidden lg:flex items-center justify-center w-7 h-7 rounded-full transition-colors mx-auto mb-4 shrink-0 ${collapsed ? 'bg-primary/20 text-primary hover:bg-primary/30' : 'bg-base-300 text-base-content/50 hover:bg-base-content/20 hover:text-base-content'}`}
